...
function clearCombobox(cmbName){
	$('#' + cmbName + ' > option').remove();
	}
...
//	<html><body>
//	<select id="country" onchange="FillStatesDropDown(this,'state');return false;"></select>
//	<select id="state" onchange="FillCityDropDown(this,'city');return false"></select>
//	<select id="city"></select>
//	<script src ="jquery.js"></script><script src ="CityState.js"></script>
//	<script>$(document).ready(function(){FillCountryDropDown('#country');});</script>
//	</body></html>
////////////////////////////////////////////////////////////
var city_cities = Object();
city_cities['Algiers'] = 'Kerala|TamilNadu|MadhyPradesh|Maharashtra|UttarPradesh|WestBengal|Uttaranchal';
city_cities['Adrar'] = 'Orisa|Behar|Assam';

function FillCityDropDown(oStateSel, oCitySel) {
    var cityArr;    
    $('#' + oCitySel + ' > option').remove();
    $('<option />', { value: '', text: '--- Select/Specify City ---' }).appendTo('#' + oCitySel);
    if (oStateSel.selectedIndex >= 0) {
        var region = oStateSel.options[oStateSel.selectedIndex].text;
        
        if (city_cities[region]) {
            cityArr = city_cities[region].split('|');
            for (var i = 0; i < cityArr.length; i++) {                
                $('<option />', { value: cityArr[i], text: cityArr[i] }).appendTo('#' + oCitySel);
            }
        }
    }
}
