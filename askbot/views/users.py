...
def show_users(request, by_group=False, group_id=None, group_slug=None):
    ...
    if askbot_settings.POINT_MODE == 'private' and sortby == 'reputation':
        sortby = 'newest'
...
def user_reputation(request, user, context):
    ...
    data = {
        ...
        'page_class': 'user-profile-page',
        'tab_name': 'reputation',
        'tab_description': _('user point'),
        'page_title': _("Profile - User's Point"),
        'reputation': reputes,
        'reps': reps
    }
    context.update(data)
...
def user(request, id, slug=None, tab_name=None):
    ...
    if askbot_settings.POINT_MODE == 'public':
        can_show_point = True
    elif askbot_settings.POINT_MODE == 'hidden':
    ...
    if can_show_point == False and tab_name == 'reputation':
        raise Http404
