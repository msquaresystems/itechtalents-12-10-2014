...
class EmpRegistrationForm(forms.Form):
    ...
    def clean_companyname(self):
        ...
        if self.cleaned_data.get('companyname', None):
            try:
                user = User.objects.get(companyname__exact=self.cleaned_data['companyname'])
...
class RecruiterFolders(forms.Form):
    ...
    #foldername = forms.CharField(widget=forms.TextInput(attrs={'class': 'span4','placeholder':'Folder name'}),required=True )
    
    #def clean_foldername(self):
      #  foldername = self.cleaned_data.get('foldername')
        #if RecruiterFolder.objects.filter(foldername=foldername):
