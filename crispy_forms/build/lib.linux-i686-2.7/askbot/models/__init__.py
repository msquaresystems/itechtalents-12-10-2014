...
def user_assert_can_upload_file(request_user):
    ...
    low_rep_error_message = _(
                        'sorry, file uploading requires point >%(min_rep)s',
...
def user_assert_can_post_text(self, text):
    ...
    if re.search(URL_RE, text):
        ...
        if self.is_authenticated() and self.reputation < min_rep:
            message = _(
                'Could not post, because your point is insufficient to publish links'
            )
            raise django_exceptions.PermissionDenied(message)
...
def user_get_point_summary(self):
    """returns human readable sentence about
    status of user's point"""
    return _("%(username)s point is %(reputation)s") % \
...
def user_fix_html_links(self, text):
    ...
    if is_simple_user and has_low_rep:
        ...
        if result != text:
            message = ungettext(
                'At least %d point point is required to post links',
                'At least %d point points is required to post links',
...
def format_instant_notification_email(
    ...
    update_data = {
        ...
        'receiving_user_point': to_user.reputation,
        'reply_by_email_point_threshold': askbot_settings.MIN_REP_TO_POST_BY_EMAIL,
        'can_reply': can_reply,
